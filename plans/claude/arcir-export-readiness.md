...
> locked decision is retained so the trade-offs aren't lost. Focuses on **R0** as the implementation
> baseline; R1–R4 are sketched to fix the shape, each getting its own detail pass before implementation.
...
| **Sample name** | `sample_name` attr → `<TITLE>` → alias/accession (the ArcId). Collision or any unmapped column → `S_0…S_N` for all columns. |
...
### Rationale & considered alternatives (locked — do not reopen without a new session)

- **Slug precedence — locked to Title-only.** The original sketch proposed `Alias → Name → Title-slug →
  accession`. Rejected: `<NAME>` on the `PRJDB5192` fixture is `Arabidopsis thaliana strain:Col-0` — an
  organism descriptor, not a project name (this is the "bad input" the user flagged); accession is stable
  but ugly and un-namelike. Erroring on a missing `Title` is preferred to silently emitting an ugly or
  wrong folder name. One function, easy to revisit if a Title-less project ever appears.
- **Sample-name precedence — locked to `sample_name` → Title → ArcId.** `sample_name` is the human-authored
  per-sample label (`WT` in the fixture) and is the best header; `<TITLE>` (`WT Col-0`) is the fallback;
  the ArcId is the last resort (see constraint 7 — it already encodes accession-then-alias).
- **Header fallback granularity — locked to all-or-nothing per file.** The alternative was to name the
  resolvable/unique columns and only `S_k` the rest. All-or-nothing reads cleaner, matches the user's
  "if unique, otherwise S_0…S_N" phrasing, and avoids a mixed header where some columns are named and
  others are `S_k`.
...
Constructors mirror the ingest readers: `Export.countSourceFromFolder path` (loose `.tsv`, as the existing
count-data unit test uses) and `Export.countSourceFromArchive path` (zipped, as `playground/ingest.fsx`
uses). The resolver matches on the `Filename` property already stored on the `CountMatrix` node, so the IR
and the raw source stay in sync by filename.

This satisfies the "pass count path(s) to F2" locked decision: the caller constructs the source from a path,
and the export function takes it alongside the IR.
...
1. `annotationText "Title" p` — the BioProject `<TITLE>` annotation (emitted by
   `BioProjectConversion.convert` via `Annotations.stringField source "Title" project.Title`).
2. Slugify: lowercase, non-`[a-z0-9]` → `_`, collapse consecutive `_`, trim leading/trailing `_`, truncate
   to 60 chars.
3. **No fallback** — if no `Title` annotation, raise (the user said "never fall back on accession").
4. Never reads `Name` (confirmed bad input on the fixture: `<NAME>` = `Arabidopsis thaliana strain:Col-0`).
...
4. On the BioSample node, try in order:
   ...
   - `annotationText "Title"` — the `<TITLE>` element annotation (emitted by `BioSampleConversion.convert`
     via `Annotations.stringField source "Title" sample.Title` → `"WT Col-0"`).
   - `Some node.Id.Value` — the `ArcId` (already encodes accession-then-alias priority via
     `Convert.entityId`; see constraint 7).
...
to `node.Id.Value` — a minor deviation from the user's 4-step chain (steps 3 and 4 merge into one),
...
1. For each run-accession column in the count file, compute `sampleName view col.RunAccession` →
   `string option`.
...
4. Otherwise → `S_0, S_1, …, S_N` for **all** run columns (0-based, matching the user's `S_0 … S_N`
   notation).
...
3. **All-or-nothing:** if every candidate is `Some` *and* the set is fully unique across the file, use the
   sample names. Otherwise fall back to positional `S_0 … S_N` for every run column.
...
already carry the 1-based `Index` (`{ Index = 2; RunAccession = "DRR072834" }` on the fixture), so the
rewrite is positional and never re-parses semantics.
...
In `tests/BioFSharp.INSDC.Tests/Tests.fs`, building on the same fixtures the ingest playground uses. The
count fixture `counts-PRJDB5192.tsv` has header `gene_id\tDRR072834\tDRR072835` and body rows
`AT1G01010\t105\t98`, `AT1G01020\t12\t15`, `AT1G01030\t0\t3`.

**IR construction** (same pattern as `GraphMlExportTests`, and the count node built exactly as
`IngestCountDataTests` does):
...
let ir =
    ...
    |> INSDC.build
let source = Export.countSourceFromFolder (Path.GetDirectoryName (TestFiles.fixture "counts-PRJDB5192.tsv"))
```

**Test cases:**

1. **Folder name** — `ExportTree.Root` = the project slug. Assert the exact slug derived from `PRJDB5192`'s
   `<TITLE>` annotation (`"The gene-body chromatin modifications dynamics mediates epigenome differentiation
   in Arabidopsis"` → slugified + truncated to 60 chars).

2. **Header, positional fallback (the real fixture)** — the count fixture has `DRR072834` (in the IR,
   resolves to `sample_name` `"WT"`) and `DRR072835` (no run record — it dangles). Because not all columns
   resolve, the all-or-nothing policy falls back to `S_0 S_1`; assert the full header line `gene_id\tS_0\tS_1`.

3. **Body untouched** — the three data rows are byte-identical to the source.
...
5. **Named path (all-mapped, all-unique)** — craft a count file / IR where every run column resolves to a
   ...
   fixture or inline TSV content; this is the case the real fixture *cannot* exercise because `DRR072835`
   dangles.)

6. **`writeTo` round-trip** — `Export.r0 … |> Export.writeTo tmpDir`, then read back from disk; assert the
   on-disk content matches the in-memory `build` output.

These lock the header-rename policy the way `feedback_ontology_changes_need_regression_test` requires for
generation changes.

### Playground

`playground/export_r0.fsx`: build the fixture IR (as `playground/ingest.fsx` does — which uses
`Ingest.countDataFromArchive` + `Ingest.incorporate`), then
