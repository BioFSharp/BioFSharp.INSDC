...
6. **Stable IRI identity for curatable units.** Objects, type assertions, properties, relations, annotations, diagnostics, reports, and transformation records that can be curated or referenced independently use the same validated absolute `Iri`/URN identity type and are serialized as keyed maps, not positional arrays. Their containing map or typed navigation case supplies the role; parallel nominal wrappers such as `AssertionId` and `RelationId` are not introduced unless a role later acquires genuinely different validation semantics.
...
13. **Terms are shared graph-level definitions.** `ArcIR` owns a `Terms: Map<Iri, OntologyTerm>` registry. Types, predicates, term-valued assertions, annotation properties, and units reference a term by IRI rather than embedding repeated term records. The registry describes terms actually used by the graph; it is not a bundled ontology, and conflicting definitions are merge conflicts rather than last-writer-wins updates.
...
F1 mints deterministic absolute IRIs/URNs from the source artifact identity, source selector, mapping/rule ID, and assertion role. Identities are never derived from the current assertion value, so a value edit cannot rename its slot. Curation-created assertions use opaque URI-shaped identities. Existing identities survive unchanged across later revisions unless an explicit identity-changing activity records their replacement.
...
- Validation detects missing endpoints, references to missing term definitions, duplicate semantic slots where forbidden, invalid IRIs, and selectors that do not resolve.
...
Add a separate navigation type to the canonical JSON persistence component:
...
`ArcJsonLocation` is not part of the ArcIR graph model and is never stored in `ArcIR`. It is an ephemeral public navigation type in the canonical JSON persistence component, used only to construct or validate a selector against a particular serialized ArcIR revision. Transformation and process artifacts retain the resulting `FragmentRef`, not the intermediate location.

`ArcIRJson.selector` is the only component that converts an `ArcJsonLocation` to an RFC 6901 URI-fragment JSON Pointer. F1, curation, F2, and UI code use this typed constructor and must not hard-code the wire layout or escaping rules.
...
The workspace POC was audited on 2026-08-24. Its executable Pyxpecto harness passes all 22 current tests, and it already models document metadata, mappings, `record_id`, CURIE maps, extension definitions, and SSSOM 1.0/1.1. It is not yet consumable by this roadmap without upstream stabilization:

- it targets `net10.0`, while the shipped ArcIR integration must remain compatible with `netstandard2.0`;
- restore currently reports an incompatible `Fable.Python`/`Fable.Core` dependency constraint, and `dotnet test` does not execute the executable test harness;
- required mapping-field validation and preservation of unknown/extension columns are incomplete;
- untested encode/decode paths currently mishandle `object_source`, entity-type values, `curation_rule` column names, nonempty extension definitions, and some mapping-set metadata;
- the suite needs the official examples plus round trips for both supported versions, CURIE maps, multivalues, extension columns, missing mandatory fields, deterministic output, and malformed input.

Fix these generic concerns in `PolyglotSSSOM`, publish or otherwise provide a reproducibly pinned compatible dependency, and run its real test/conformance entry point in that library's CI. Do not copy fixes into BioFSharp or add a fallback tabular implementation here.
...
- ArcIR integration tests consume and emit SSSOM exclusively through `PolyglotSSSOM`, resolve declared CURIEs, retain mapping-record identity and rich mapping metadata, and bind used records to exact artifact revisions.
...
type FieldDisposition =
    ...
    | Unsupported of diagnostic: Iri
    | Failed of diagnostic: Iri
...
- the exact source, mapping-set, ArcIR, diagnostic, transformation, target, and review artifacts to retain, including expected branches/revisions and digest bindings;
